...
elseif ($structure == 0) {
  ...
  $structurequery = "select structure.name, concat('R=" . $region . "+S=" . $subregion . "+P=" . $place . "+M=', structure.id) as link from structure where structure.parentid = " . $place;
  ...
  $t->set_var('body', $body);
  $imagequery = "select fname from images where pagekey = 'R=" . $region ."+S=" . $subregion . "+P=" . $place . "+M=" . $structure . "'";
  $imageresult = $dbconn->Execute($imagequery);
  $imagecount = $imageresult->RecordCount();
  if ($imagecount > 0) {
    $t->set_var('imagecount', $imagecount);
